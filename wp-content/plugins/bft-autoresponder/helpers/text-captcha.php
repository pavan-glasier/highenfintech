<?php
class BFTTextCaptcha {
	// verify the captcha
	static function verify($question, $answer) {
		$answer = stripslashes($answer);
		
		$captcha_questions = get_option('bft_text_captcha');
	...
	static function generate() {
		...
		return trim($q)." <input type='text' name='bft_text_captcha_answer'>\n<input type='hidden' name='bft_text_captcha_question' value=\"".base64_encode(trim($q))."\">"; 
	}
}
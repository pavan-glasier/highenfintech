...
if (!class_exists('wpMailPlugin')) {
	class wpMailPlugin extends wpMailCheckinit {
			...
			if ($useragents = $detect -> getUserAgents()) {
				foreach ($useragents as $useragent => $regex) {					
					if (!empty($_SERVER['HTTP_USER_AGENT'])) {
						if (strpos( sanitize_text_field(wp_unslash($_SERVER['HTTP_USER_AGENT'])), $useragent) !== false) {
							$agent = $useragent;
						}
					}
				}
	    ...
	    function ajax_get_country() {		    
		    ...
		    if (current_user_can('newsletters_subscribers')) {
			    if (!empty($_POST['subscriber_id'])) {
				    $subscriber_id = (int) sanitize_text_field(wp_unslash($_POST['subscriber_id']));
	    ...
	    function ajax_getposts() {
		    ...
		    if (!empty($_POST)) {
			    ...
			    if (!empty($_POST['posttype'])) { $arguments['post_type'] = sanitize_text_field(wp_unslash($_POST['posttype'])); }
			    if (!empty($_POST['category'])) { $arguments['category'] = array_map('sanitize_text_field', $_POST['category']); }
			    if (!empty($_POST['number'])) { $arguments['numberposts'] = sanitize_text_field(wp_unslash($_POST['number'])); }
			    if (!empty($_POST['orderby'])) { $arguments['orderby'] = sanitize_text_field(wp_unslash($_POST['orderby'])); }
			    if (!empty($_POST['order'])) { $arguments['order'] = sanitize_text_field(wp_unslash($_POST['order'])); }
	    ...
	    function ajax_forms_createform() {
		    ...
		    if (!empty($_POST)) {	
			    ...
			    if (current_user_can('newsletters_forms')) {	    
				    if ($this -> Subscribeform() -> save(map_deep(wp_unslash($_POST[$this -> Subscribeform() -> model]), 'sanitize_text_field'))) {
					    $success = true;
		...
		function ajax_executemultiple() {
			...
			$subscribers = (object) map_deep(wp_unslash($_REQUEST['subscribers']), 'sanitize_text_field');

			if (!empty($subscribers)) {
				$historyquery = "SELECT id, message, subject FROM " . $wpdb -> prefix . $this -> History() -> table . " WHERE id = '" . esc_sql(sanitize_text_field(wp_unslash($_REQUEST['history_id']))) . "' LIMIT 1";
				...
				if (!empty($history)) {
					foreach ($subscribers as $subscriber_request) {
						...
						$history_id = sanitize_text_field(wp_unslash($_REQUEST['history_id']));
			...
			if (!empty($subscribers)) {
				...
				if (!empty($history)) {
					foreach ($subscribers as $subscriber_request) {
						...
						$post_id = sanitize_text_field(wp_unslash($_REQUEST['post_id']));
						$theme_id = sanitize_text_field(wp_unslash($_REQUEST['theme_id']));
						$senddate = sanitize_text_field(wp_unslash($_REQUEST['senddate']));
						$shortlinks = true;
				...
				if ($form = $this -> Subscribeform() -> find(array('id' => $id))) {	
					...
					<html>
						<head>
							...
							<title><?php echo urldecode(sanitize_text_field(wp_unslash($_GET['title']))); ?></title>
		...
		function ajax_mailapi_amazonses_action() {
			...
			if (!empty($_REQUEST['ac'])) {				
				require($this -> plugin_base() . DS . 'vendor' . DS . 'autoload.php');
				$ses = new SimpleEmailService(sanitize_text_field(wp_unslash($_REQUEST['key'])), sanitize_text_field(wp_unslash($_REQUEST['secret'])), 'email.' . sanitize_text_field(wp_unslash($_REQUEST['region']) . '.amazonaws.com', false));
	...
	<?php // phpcs:ignore ?>
		...
		function ajax_newsletters_autosave() {
	...
	<?php // phpcs:ignore ?>
	    ...
	    function ajax_history_download() {
			...
			if (!empty($id)) {			
				...
				ob_start();
				echo wp_kses_post($this -> process_set_variables($subscriber, $user, $output, $email -> id));
				$output = ob_get_clean();
...
// phpcs:disable
			...
			switch ($method) {
				...
				default						:
					if (!empty($_GET['subscriberauth'])) {
						$_COOKIE['subscriberauth'] = sanitize_text_field(wp_unslash($_GET['subscriberauth']));
					}
		...
		function paginate($model = null, $fields = '*', $sub = null, $conditions = false, $searchterm = null, $perpage = 10, $order = array('modified', "DESC"), $conditions_and = null) {
			...
			if (!empty($model)) {
				...
				$page = (empty($_GET[$this -> pre . 'page'])) ? 1 :  sanitize_text_field(wp_unslash($_GET[$this -> pre . 'page']));
		...
		function custom_admin_pointers_footer() {
		    ...
		    (function($) {
		        ...
		        foreach ($admin_pointers as $pointer => $array) {
		            ...
		            $('<?php echo esc_html( $array['anchor_id']); ?>').pointer( {
		                content: '<?php echo esc_html( $array['content']); ?>',
		                position: {
		                    edge: '<?php echo esc_html( $array['edge']); ?>',
		                    align: '<?php echo esc_html( $array['align']); ?>'
		                },
		                close: function() {
		                    $.post( ajaxurl, {
		                        pointer: '<?php echo esc_html( $pointer); ?>',
		...
		function check_tables() {
			...
			if (!empty($this -> models)) {
				foreach ($this -> models as $model) {
					...
					$this -> {$model}() -> fields = isset($this -> {$model}() -> fields) ? apply_filters('newsletters_db_table_fields_new', $this -> {$model}() -> fields, $model) : null;
					$this -> tablenames[$this -> pre . $this -> {$model}() -> controller] = $wpdb -> prefix . $this -> {$model}() -> table;
					...
					switch ($field -> type) {
						...
						case 'pre_gender'		:
							...
							<select<?php echo esc_html( $placeholder); ?> class="<?php echo esc_html($this -> pre); ?> <?php echo esc_html($this -> pre); ?>pregender<?php echo ((!empty($Subscriber -> errors[$field -> slug])) ? ' newsletters_fielderror' : ''); ?>" style="width:auto;" id="<?php echo esc_html($this -> pre); ?>-<?php echo esc_html( $optinid); ?><?php echo esc_html( $field -> slug); ?>" <?php echo esc_html( $Html -> tabindex($optinid)); ?> name="<?php echo esc_html( $fieldname); ?>">
								...
								<option <?php echo (!empty($fieldvalue) && $fieldvalue == "other") ? 'selected="selected"' : ''; ?> value="other"><?php esc_html_e('Other', 'wp-mailinglist'); ?></option>
							</select>
			...
			if (!empty($subscriber) || !empty($user)) {
				...
				if (!empty($subscriber)) {
					...
					if (!empty($alllists) && $alllists == true) {
						...
						$history = $this -> History() -> find(array('id' => $history_id));
						$history_lists = !empty($history) ?  maybe_unserialize($history -> mailinglists) : '';
					...
					if (!empty($regs[1])) {
						...
						foreach ($results as $rkey => $result) {							
							if (apply_filters('wpml_hashlink_loop', true, $result, $regs)) {								
								...
								$hashlink = $this -> hashlink($result, $history_id, $subscriber -> id, $user_id);								
			...
			if (!empty($match)) {
				...
				if ($emails = $this -> History() -> find_all(array('sent' => "> 0"), false, array('modified', "DESC"))) {
					...
					foreach ($emails as $email) {
						...
						<h3><a href="<?php echo esc_url_raw($Html -> retainquery('newsletters_method=newsletter&id=' . $email -> id, home_url())); ?>" title="<?php echo esc_html( $email -> subject); ?>"><?php echo esc_html( $email -> subject); ?></a></h3>
						<div><small><?php esc_html_e('Sent on', 'wp-mailinglist'); ?> : <?php echo esc_html( $email -> modified); ?></small></div>
						<?php echo wp_kses_post($this -> strip_set_variables($email -> message)); ?>
		...
		function ajax_latestposts_preview() {
			...
			if (current_user_can('newsletters_settings')) {
		    	...
		    	if ($content = $this -> latestposts_hook($id, true)) {
					...
					esc_html_e('Latest posts preview could not load', 'wp-mailinglist');
				}
			} else {esc_html_e('You do not have permission', 'wp-mailinglist');
			}
		...
		function updating_plugin() {
			...
			if (version_compare($this -> version, $cur_version) === 1) {
				...
				if (version_compare($cur_version, "4.6.20") < 0) {
					$this -> update_options();
					$this -> check_tables();
					$this -> initialize_default_themes();

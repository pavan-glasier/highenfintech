...
Version: 4.7.9.3
Description: This newsletter software allows users to subscribe to multiple mailing lists on your WordPress website. Send newsletters manually or from posts, manage newsletter templates, view a complete history with tracking, import/export subscribers, accept paid subscriptions and much more.
Author: Tribulant
...
if (!class_exists('wpMail')) {
    class wpMail extends wpMailPlugin {
        ...
        function admin_notices() {
            ...
            if (is_admin() && !defined('DOING_AJAX')) {
                ...
                if ((!empty($page) && in_array($page, (array) $this -> sections)) || (!empty($pagenow) && $pagenow == "newsletter")) {
                    $Html -> wp_has_current_submenu($page);
                }
        ...
        function init() {
            ...
            if (!empty($_GET[$this -> pre . 'link']) || !empty($_GET['newsletters_link'])) {
                ...
                if ($link = $this -> Link() -> find(array('hash' => $link_hash))) {
                    ...
                    $email_conditions = array('history_id' => sanitize_text_field(wp_unslash($_GET['history_id'])));
                    if (!empty($_GET['subscriber_id'])) { $email_conditions['subscriber_id'] = sanitize_text_field(wp_unslash($_GET['subscriber_id'])); }
                    if (!empty($_GET['user_id'])) { $email_conditions['user_id'] = sanitize_text_field(wp_unslash($_GET['user_id'])); }
        ...
        function save_newsletter($post_id = null, $post = null, $onlyupdatehistory = false) {
            ...
            $subject = wp_kses_post(sanitize_text_field(wp_unslash($_POST['post_title'])));
                        ...
                        if (!empty($errors)) {
                            ...
                            if (!empty($newsletters_mailinglists) || !empty($newsletters_roles)) {
                                ...
                                if ($sendingprogress == "Y") {
                                    ...
                                    $content = wp_kses_post($_POST['content']);
                                    $this -> render('send-post', array('subscribers' => $datasets, 'subject' => $subject, 'content' => $content, 'attachments' => $newattachments, 'post_id' => $post_id, 'history_id' => $history_id, 'theme_id' => sanitize_text_field(wp_unslash($_POST['theme_id'])), true, 'admin'));
                                    $dontrendersend = true;
                                } else {
                                    $this -> qp_save();
                                    $this -> qp_dispatch();
        ...
        function save_post($post_id = null, $post = null) {
            ...
            if (!empty($post_id) && !empty($post)) {
                $newsletters_subject = sanitize_text_field(wp_unslash($_POST['newsletters_subject']));
        ...
        function admin_forms() {
            ...
            $form_id = (!empty($_GET['id'])) ? sanitize_text_field(wp_unslash($_GET['id'])) : false;
            $method = sanitize_text_field(isset($_GET['method']) ? $_GET['method'] : "");
            switch ($method) {
                case 'save'								:
                    if (!empty($_POST)) {
                ...
                default				:
                    if (!empty($_POST['newsletter'])) {
                        ...
                        if (empty($errors)) {
                            ...
                            $defaulttexton = sanitize_text_field(wp_unslash($_POST['defaulttexton']));
                            if (!empty($defaulttexton) && !empty($_POST['customtext'])) {
                                $this -> update_option('defaulttexton', true);
                                $this -> update_option('defaulttextversion', sanitize_textarea_field(wp_unslash($_POST['customtext'])));
                            } else {
                                $this -> delete_option('defaulttexton');
                                $this -> delete_option('defaulttextversion');
                            }
                            ...
                            if (empty($_POST['preview']) && empty($_POST['draft'])) {
                                if (!empty($_POST)) {
                                    if (!empty($errors)) {
                                        ...
                                        $history_data = array(
                                            ...
                                            'countries'			=> 	array_map('sanitize_text_field', wp_unslash($_POST['countries'])),
                                            ...
                                            'senddate'			=>	sanitize_text_field(wp_unslash($_POST['senddate'])),
                                            'scheduled'			=>	sanitize_text_field(wp_unslash($_POST['scheduled'])),
                                ...
                                $history_data = array(
                                    ...
                                    'daterangefrom'		=>	sanitize_text_field(wp_unslash($_POST['daterangefrom'])),
                                    'daterangeto'		=>	sanitize_text_field(wp_unslash($_POST['daterangeto'])),
                                    'countries'			=>	map_deep(wp_unslash($_POST['countries']), 'sanitize_text_field'),
                                    'selectedcountries'	=>	maybe_serialize(map_deep(wp_unslash($_POST['selectedcountries']), 'sanitize_text_field')),
                                    'mailinglists'		=>	maybe_serialize(map_deep(wp_unslash($_POST['mailinglists']), 'sanitize_text_field')),
                                    'groups'			=>	maybe_serialize(map_deep(wp_unslash($_POST['groups']), 'sanitize_text_field')),
                                    'roles'				=>	maybe_serialize(map_deep(wp_unslash($_POST['roles']), 'sanitize_text_field')),
                                ...
                                if (!empty($_POST['previewemail'])) {
                                    $emails = explode(",", sanitize_text_field(wp_unslash($_POST['previewemail'])));
            ...
            switch ($method) {
                case 'save'					:
                    if (!empty($_POST)) {
                        ...
                        if ($this -> Autoresponder() -> save(map_deep(wp_unslash($_POST), 'sanitize_text_field'))) {
                ...
                default						:
                    ...
                    $perpage = (isset($_COOKIE[$this -> pre . 'autorespondersperpage'])) ? (int) $_COOKIE[$this -> pre . 'autorespondersperpage'] : 15;
                    $searchterm = (!empty($_GET[$this -> pre . 'searchterm'])) ? sanitize_text_field(wp_unslash($_GET[$this -> pre . 'searchterm'])) : false;
                    $searchterm = (!empty($_POST['searchterm'])) ? sanitize_text_field(wp_unslash($_POST['searchterm'])) : $searchterm;
                    ...
                    if (!empty($_GET['filter'])) {
                        ...
                        if (!empty($_GET['list'])) {
                            switch ($_GET['list']) {
                                ...
                                default						:
                                    $dojoin = true;
                                    $conditions_and[$autoresponderslist_table . '.list_id'] = sanitize_text_field(wp_unslash($_GET['list']));
                                    break;
                            }
                        }
                    ...
                    if (!empty($_GET['showall'])) {
                        ...
                        if ($dojoin) {
                            ...
                            $autoresponders = isset($data[$this -> Autoresponder() -> model]) ? $data[$this -> Autoresponder() -> model] : [];
                        }
                    }
                    ...
                    $data['Paginate'] = isset($data['Paginate']) ? $data['Paginate'] : '';
        ...
        function admin_autoresponderemails() {
            ...
            switch ($method) {
                ...
                case 'mass'					:
                    ...
                    break;
                default						:
                    $perpage = (isset($_COOKIE[$this -> pre . 'autoresponderemailsperpage'])) ? (int) $_COOKIE[$this -> pre . 'autoresponderemailsperpage'] : 15;
                    $searchterm = (!empty($_GET[$this -> pre . 'searchterm'])) ? sanitize_text_field(wp_unslash($_GET[$this -> pre . 'searchterm'])) : false;
                    $searchterm = (!empty($_POST['searchterm'])) ? sanitize_text_field(wp_unslash($_POST['searchterm'])) : $searchterm;
                    ...
                    if (!empty($_GET['status'])) {
                        $_COOKIE[$this -> pre . 'autoresponderemailsfilter_status'] = sanitize_text_field(wp_unslash($_GET['status']));
                    }
                    ...
                    if (isset($_COOKIE[$this -> pre . 'autoresponderemailsfilter_autoresponder_id'])) {
                        if (empty($conditions['status'])) {
                            $conditions['autoresponder_id'] = (int) $_COOKIE[$this -> pre . 'autoresponderemailsfilter_autoresponder_id'];
                        } else {
                            $conditions['status'] .= "' AND autoresponder_id = '" . (int) $_COOKIE[$this -> pre . 'autoresponderemailsfilter_autoresponder_id'] . "";
                        }
                    }
            ...
            switch ($method) {
                case 'save'			:
                    if (!empty($_POST)) {
                        ...
                        if (!empty($_GET['group_id'])) { $Mailinglist -> data -> group_id = sanitize_text_field(wp_unslash($_GET['group_id'])); }
            ...
            switch ($method) {
                case 'save'			:
                    if (!empty($_POST)) {
                        ...
                        if (!empty($_GET['mailinglist_id'])) {
                            ...
                            $Subscriber -> data -> mailinglists = array(sanitize_text_field(wp_unslash($_GET['mailinglist_id'])));
                        }
                ...
                default			:
                    ...
                    if (!empty($newsletters_filter_subscribers)) {
                        ...
                        //** registered filter
                        $newsletters_filter_subscribers_registered = (empty($_GET['registered'])) ? sanitize_text_field(wp_unslash($_COOKIE['newsletters_filter_subscribers_registered'])) : sanitize_text_field(wp_unslash($_GET['registered']));
                        ...
                        //** country filter
                        $newsletters_filter_subscribers_country = (empty($_GET['country'])) ? sanitize_text_field(wp_unslash($_COOKIE['newsletters_filter_subscribers_country'])) : sanitize_text_field(wp_unslash($_GET['country']));
        ...
        function admin_themes() {
            ...
            switch ($method) {
                ...
                case 'mass'			:
                    ...
                    break;
                default				:
                    $perpage = (empty($_COOKIE[$this -> pre . 'themesperpage'])) ? 15 :  sanitize_text_field(wp_unslash($_COOKIE[$this -> pre . 'themesperpage']));
                    $searchterm = (!empty($_GET[$this -> pre . 'searchterm'])) ? sanitize_text_field(wp_unslash($_GET[$this -> pre . 'searchterm'])) : false;
                    $searchterm = (!empty($_POST['searchterm'])) ? sanitize_text_field(wp_unslash($_POST['searchterm'])) : $searchterm;

                    if (!empty($_POST['searchterm'])) {
                        $this -> redirect($this -> url . '&' . $this -> pre . 'searchterm=' . $searchterm);
                    }
        ...
        function admin_templates() {
            ...
            //get the page method
            $method = sanitize_text_field(isset($_GET['method']) ? $_GET['method'] : "");
            $method = (empty($method)) ? preg_replace("/newsletters\-templates\-/si", "", sanitize_text_field(wp_unslash($_GET['page']))) : $method;
            ...
            switch ($method) {
                case 'save'			:
                    ...
                    if (!empty($_POST)) {
                        if ($this -> Template() -> save(map_deep(wp_unslash($_POST), 'sanitize_text_field'))) {
                            $message = 10;
                ...
                case 'mass'						:
                    if (!empty($_POST['action'])) {
                        if (!empty($_POST['templateslist'])) {
                            if ($this -> Template() -> delete_array(map_deep(wp_unslash($_POST['templateslist']), 'sanitize_text_field'))) {
                                $msg_type = 'message';
                                $message = 18;
                            } else {
                                $msg_type = 'error';
                    ...
                    break;
                default 						:
                    $perpage = (empty($_COOKIE[$this -> pre . 'templatesperpage'])) ? 15 : (int) $_COOKIE[$this -> pre . 'templatesperpage'];
                    $searchterm = (!empty($_GET[$this -> pre . 'searchterm'])) ? sanitize_text_field(wp_unslash($_GET[$this -> pre . 'searchterm'])) : false;
                    $searchterm = (!empty($_POST['searchterm'])) ? sanitize_text_field(wp_unslash($_POST['searchterm'])) : $searchterm;
        ...
        function admin_mailqueue() {
            ...
            switch ($method) {
                ...
                case 'deletebatch'			:
                    ...
                    break;
                case 'sendbatch'			:
                    $batchkey = array_map('sanitize_text_field', $_GET['batch']);
                    if (!empty($batchkey)) {
        ...
        function admin_history() {
            ...
            switch ($method) {
                ...
                case 'exportsent'		:
                        ...
                        if ($emails = $Db -> find_all(array('history_id' => esc_sql((int) $_GET['history_id'])), false, array('modified', "DESC"))) {
                            ...
                            if (!empty($data)) {
                                ...
                                if ($fh = fopen($exportfull, "w")) {
                                    ...
                                    $exportfileabs = $Html -> uploads_url() . '/' . $exportfile;
                                    $msg_type = 'message';
                                    $this -> redirect(admin_url('admin.php?page=' . $this -> sections -> history . '&method=view&id=' . sanitize_text_field(wp_unslash($_GET['history_id']) . '&newsletters_exportlink=' . $exportfile)));
                                } else {
                                    $msg_type = 'error';
                                    $message = sprintf(__('CSV file could not be created, please check write permissions on "%s" folder.', 'wp-mailinglist'), $exportpath);
                                }
                    ...
                    $this -> redirect(admin_url("admin.php?page=" . $this -> sections -> history . "&method=view&id=" . sanitize_text_field(wp_unslash($_GET['history_id'])), $msg_type, $message));
					break;
                default					:
            ...
            switch ($method) {
                ...
                case 'mass'				:
                    ...
                    if (!empty($_POST['fieldslist'])) {
                        if (!empty($_POST['action'])) {
                            $fields = map_deep(wp_unslash($_POST['fieldslist']), 'sanitize_text_field');
                            $mailinglists = map_deep(wp_unslash($_POST['mailinglists']), 'sanitize_text_field');
                            $msg_type = 'message';
                ...
                default					:
                    ...
                    if (!empty($_GET['showall'])) {
                        ...
                        $perpage = (!empty($_COOKIE[$this -> pre . 'fieldsperpage'])) ? (int) $_COOKIE[$this -> pre . 'fieldsperpage'] : 15;
                        $searchterm = (empty($_GET[$this -> pre . 'searchterm'])) ? '' :  sanitize_text_field(wp_unslash($_GET[$this -> pre . 'searchterm']));
                        $searchterm = (empty($_POST['searchterm'])) ? $searchterm :  sanitize_text_field(wp_unslash($_POST['searchterm']));
        ...
        function admin_config() {
            ...
            $method = esc_html(isset($_GET['method']));
            $method = (empty($method)) ? false : $method;

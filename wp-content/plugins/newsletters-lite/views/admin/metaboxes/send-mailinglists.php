	        ...
	        <div class="misc-pub-section">
	        	...
	        	<div id="daterange_div" style="display:<?php echo (!empty($_POST['daterange']) && $_POST['daterange'] == "Y") ? 'block' : 'none'; ?>;">
	        		<p>
	        			...
	        			<input placeholder="<?php echo esc_attr(wp_unslash($Html -> gen_date("Y-m-d", strtotime("-1 month")))); ?>" onblur="update_subscribers();" onkeyup="update_subscribers();" type="text" name="daterangefrom" value="<?php echo esc_attr(sanitize_text_field(wp_unslash($_POST['daterangefrom']))); ?>" id="daterangefrom" class="widefat" style="width:120px;" />
	        		</p>
	        		<p>
	        			<label for="daterangeto"><?php esc_html_e('To Date', 'wp-mailinglist'); ?></label>
	        			<input placeholder="<?php echo esc_attr(wp_unslash($Html -> gen_date("Y-m-d"))); ?>" onblur="update_subscribers();" onkeyup="update_subscribers();" type="text" name="daterangeto" value="<?php echo esc_attr(sanitize_text_field(wp_unslash($_POST['daterangeto']))); ?>" id="daterangeto" class="widefat" style="width:120px;" />
	        		</p>
	        	</div>
	        	
	        	<script type="text/javascript">
	        	jQuery(document).ready(function() {
		        	jQuery('#daterangefrom').datepicker({showButtonPanel:true, numberOfMonths:1, changeMonth:true, changeYear:true, defaultDate:"<?php echo esc_js(sanitize_text_field(wp_unslash($_POST['daterangefrom']))); ?>", dateFormat:"yy-mm-dd"});
		        	jQuery('#daterangeto').datepicker({showButtonPanel:true, numberOfMonths:1, changeMonth:true, changeYear:true, defaultDate:"<?php echo esc_js(sanitize_text_field(wp_unslash($_POST['daterangeto']))); ?>", dateFormat:"yy-mm-dd"});
	        	});
	        	</script>
	        </div>
	    <?php endif; ?>
		                                ...
		                                switch ($field -> type) {
											...
											case 'pre_gender'		:
		                                        ...
		                                        <select style="max-width:250px;" name="fields[<?php echo esc_html( $field -> slug); ?>]" id="fields_<?php echo esc_html( $field -> id); ?>" onchange="update_subscribers();">
		                                            ...
		                                            <option <?php echo (!empty($_POST['fields'][$field -> slug]) && $_POST['fields'][$field -> slug] == "other") ? 'selected="selected"' : ''; ?> value="other"><?php esc_html_e('Other', 'wp-mailinglist'); ?></option>
		                                        </select>

...
<div class="submitbox">
	<div class="misc-pub-section misc-pub-section-last">        
        <div class="scroll-list">	        
            <?php if (apply_filters($this -> pre . '_admin_nonetheme', true)) : ?><div><label><input <?php echo ($_POST['theme_id'] == 0 || (empty($_POST['theme_id']) && $this -> default_theme_id('sending') == "")) ? 'checked="checked"' : ''; ?> type="radio" name="theme_id" value="0" id="theme0" /> <?php esc_html_e('NONE', 'wp-mailinglist'); ?></label></div><?php endif; ?>
            <?php if ($themes = $Theme -> select()) : ?>
                <?php $default_theme_id = $this -> default_theme_id('sending'); ?>
                <?php foreach ($themes as $theme_id => $theme_title) : ?>
                    <div><label><input <?php echo ((!empty($_POST['theme_id']) && $_POST['theme_id'] == $theme_id) || (empty($_POST['theme_id']) && $_POST['theme_id'] != "0" && $theme_id == $default_theme_id)) ? 'checked="checked"' : ''; ?> type="radio" name="theme_id" value="<?php echo esc_attr($theme_id); ?>" id="theme<?php echo esc_html( $theme_id); ?>" /> <?php echo esc_html($theme_title); ?></label>

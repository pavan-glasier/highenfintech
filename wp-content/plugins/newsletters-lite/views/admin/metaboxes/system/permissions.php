        	...
        	<tr>
        		...
        		<td>
        			...
        			<?php foreach ($wp_roles -> role_names as $role_key => $role_name) : ?>
        				<label><input <?php echo (!empty($permissions['newsletters_admin_send_sendtoroles']) && in_array($role_key, $permissions['newsletters_admin_send_sendtoroles'])) ? 'checked="checked"' : ''; ?> type="checkbox" name="permissions[newsletters_admin_send_sendtoroles][]" value="<?php echo esc_html( $role_key); ?>" id="permissions_<?php echo esc_html( $block); ?>_<?php echo esc_html( $role_key); ?>" /> <?php echo esc_html($role_name); ?></label><br/>
        			<?php endforeach; ?>
        		</td>
        	</tr>
        </tbody>

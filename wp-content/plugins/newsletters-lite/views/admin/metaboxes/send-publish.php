	...
	<table class="form-table">
		<tbody>
				...
				<td>
					...
					<select class="widefat" style="width:auto;" id="post_status" name="post_status">
						<?php foreach ($statuses as $key => $val) : ?>
							...
							<option <?php echo esc_html( $sel); ?> value="<?php echo esc_html($key); ?>"><?php echo esc_html( $val); ?></option>
						<?php endforeach; ?>
					</select>
				</td>
			</tr>
			...
			<tr>
				...
				<td>
					<input class="widefat" type="text" size="25" name="post_slug" value="<?php echo esc_attr(sanitize_text_field(wp_unslash($_POST['post_slug']))); ?>" />
					<span class="howto"><small><?php esc_html_e('(optional)', 'wp-mailinglist'); ?></small> <?php esc_html_e('Post slug to use for this post', 'wp-mailinglist'); ?></span>
				</td>
			</tr>
		</tbody>
	</table>

	...
	<form action="?page=<?php echo esc_html( $this -> sections -> fields); ?>&amp;method=save" method="post" id="Field.saveform">
			...
			<tbody>
				<tr>
					...
					<td>
                        ...
                        <?php echo esc_html( $Html -> field_error('Field[title]')); ?>
                    </td>
				</tr>
				<?php if (empty($slug) || ($slug != 'email' && $slug != "list")) : ?>
	                <tr>
	                    ...
	                    <td>
	                    	<input placeholder="<?php echo esc_attr(wp_unslash(__('Enter field slug for database and shortcodes use here', 'wp-mailinglist'))); ?>" id="Field_slug" type="text" class="widefat" name="Field[slug]" value="<?php echo esc_attr(wp_unslash($Html -> field_value('Field[slug]'))); ?>" />
	                        ...
	                        <?php endif; ?>
	                        <span class="howto"><?php esc_html_e('Only use letters, lowercase, no spaces or other characters, please.', 'wp-mailinglist'); ?></span>
	                    </td>
	                </tr>
	            <?php else : ?>
		...
		<?php if (empty($slug) || ($slug != 'email' && $slug != "list")) : ?>
			<table class="form-table">
				<tbody>
					<tr>
						<th><label for="Field.type"><?php esc_html_e('Field Type', 'wp-mailinglist'); ?></label></th>
						<td>
							<?php 
				...
				<tbody>
					<tr>
						...
						<td>
							...
							<?php echo esc_html( $Html -> field_error('Field[hidden_value]')); ?>
						</td>
					</tr>
				</tbody>
			</table>
						...
						<td>
							<?php if ($this -> language_do()) : ?>
								<div id="tabs_errormessage">
									...
									<?php $tabs_errormessage = 1; ?>
									<?php foreach ($languages as $language) : ?>
										<div id="tabs_errormessage_<?php echo esc_html( $tabs_errormessage); ?>">
											<input type="text" class="widefat" name="Field[errormessage][<?php echo esc_html( $language); ?>]" value="<?php echo esc_attr(wp_unslash($this -> language_use($language, $Field -> data -> errormessage))); ?>" id="Field_errormessage_<?php echo esc_html( $tabs_errormessage); ?>" />
										</div>
										<?php $tabs_errormessage++; ?>
									<?php endforeach; ?>
								</div>
				...
				<tbody>
					<tr>
						...
						<td>
							...
							<?php echo esc_html( $Html -> field_error('Field[display]')); ?>
						</td>
					</tr>
				</tbody>
			</table>
						...
						<tr>
							...
							<td>
								...
								<?php if (!empty($lists)) : ?>
									...
									<?php echo esc_html( $Html -> field_error('Field[mailinglists]')); ?>
								<?php else : ?>

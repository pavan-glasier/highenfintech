...
<div class="wrap newsletters">
	...
	<form id="posts-filter" action="?page=<?php echo esc_html( $this -> sections -> fields); ?>" method="post">
		...
		<p class="search-box">
			<input type="text" name="searchterm" id="post-search-input" class="search-input" value="<?php echo (empty($_POST['searchterm'])) ? esc_attr(sanitize_text_field(wp_unslash($_GET[$this -> pre . 'searchterm']))) : esc_attr(sanitize_text_field(wp_unslash($_POST['searchterm']))); ?>" />
			<input class="button" name="search" type="submit" value="<?php esc_html_e('Search Fields', 'wp-mailinglist'); ?>" />
		</p>
	</form>

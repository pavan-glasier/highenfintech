...
class wpmlFormHelper extends wpMailPlugin {
	...
	function file($name = null, $options = array()) {
		...
		?><input class="<?php echo esc_html( $class); ?>" style="width:<?php echo esc_html( $width); ?>;" type="file" name="<?php echo esc_html($name); ?>" id="<?php echo esc_html($Html -> field_id($name)); ?>" /><?php
		
		if ($error == true) {
			echo esc_html( $Html -> field_error($name));
		}
	...
	function hidden($name = null, $options = array()) {
		...
		?><input type="hidden" name="<?php echo esc_html($name); ?>" value="<?php echo esc_attr(wp_unslash($Html -> field_value($name))); ?>" /><?php
	...
	function text($name = null, $options = array()) {
		...
		?><input placeholder="<?php echo esc_attr(wp_unslash($placeholder)); ?>" autocomplete="<?php echo esc_html( $autocomplete); ?>" <?php echo (empty($tabindex)) ? '' : 'tabindex="' . esc_attr(wp_unslash($tabindex)) . '"'; ?> class="<?php echo esc_html( $class); ?>" style="width:<?php echo esc_html( $width); ?>;" id="<?php echo esc_html($id); ?>" type="text" name="<?php echo esc_html($name); ?>" value="<?php echo esc_attr(wp_unslash($Html -> field_value($name))); ?>" /><?php
	...
	function textarea($name = null, $options = array()) {
		...
		?><textarea style="width:<?php echo esc_html( $width); ?>;" class="<?php echo esc_html( $class); ?>" name="<?php echo esc_html($name); ?>" id="<?php echo esc_html($Html -> field_id($name)); ?>" cols="<?php echo esc_html( $cols); ?>" rows="<?php echo esc_html( $rows); ?>"><?php echo wp_kses_post( wp_unslash($Html -> field_value($name))) ?></textarea><?php

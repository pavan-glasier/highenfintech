	...
	class wpmlShortcodeHelper extends wpMailPlugin {
		...
		function woocommerce_products($atts = array()) {
			...
			if ($wc_query -> have_posts()) {
				...
				while ($wc_query -> have_posts()) {
	                ...
	                $c++;
	                if ($c%$breaker == 0 || $c == count($wc_query -> posts_count)) {
		             	$output .= '</tr>'; 
		            }
                }
		...
		function authenticate($atts = array(), $content = null) {
			...
			if (!empty($current_subscriber)) {
				...
				$link = '<a href="' . $url . '" class="' . esc_attr(wp_unslash($class)) . '" style="' . esc_attr(wp_unslash($style)) . '">' . esc_attr(wp_unslash(esc_html($text))) . '</a>';

...
class wpmlPost extends wpmlDbHelper {
	...
	function save($data = array(), $validate = true) {
		...
		if (!empty($data)) {
			if ($validate == true) {
				if (empty($data['post_id'])) { $this -> errors[] = __('No post was specified', 'wp-mailinglist'); }
				if (empty($data['sent'])) { $this -> errors[] = __('No sent status was specified', 'wp-mailinglist'); }
			}

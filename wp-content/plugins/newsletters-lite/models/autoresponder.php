	...
	class wpmlAutoresponder extends wpmlDbHelper {
		...
		function validate($data = array()) {
			...
			if (!empty($data)) {
				...
				if (empty($lists) && empty($forms)) { $this -> errors['lists'] = __('Please select subscribe form/s OR mailing list/s.', 'wp-mailinglist'); }
				
				if (empty($newsletter)) {
					$this -> errors['newsletter'] = __('Please choose a newsletter type.', 'wp-mailinglist');	
				} else {
					if ($newsletter == "new") {
						if (empty($nnewsletter['subject'])) { $this -> errors['nnewsletter_subject'] = __('Please fill in a subject.', 'wp-mailinglist'); }	
						if (empty($_POST['content'])) { $this -> errors['nnewsletter_content'] = __('Please fill in content for this newsletter.', 'wp-mailinglist'); }
					} else {
						if (empty($history_id)) { $this -> errors['history_id'] = __('Please select a history email.', 'wp-mailinglist'); }
					}
				}
				
				if (!empty($sendauto)) {
					if (empty($delay) && $delay != "0") { $this -> errors['delay'] = __('Please fill in a send delay.', 'wp-mailinglist'); }
				} else {

		...
		function validate($data = array()) {
			...
			if (!empty($data)) {
				...
				if (empty($type)) { $this -> errors['type'] = __('Please choose a submission type', 'wp-mailinglist'); }
					...
					switch ($type) {
						case 'upload'		:
							$_FILES['upload'] = map_deep(wp_unslash($_FILES['upload']), 'sanitize_text_field');
							if ($_FILES['upload']['error'] > 0) {
								$this -> errors['upload'] = $Html -> file_upload_error(sanitize_text_field(wp_unslash($_FILES['upload']['error'])));
							} else {							
								if (empty($_FILES['upload']['name'])) { $this -> errors['upload'] = __('Please choose an HTML file for uploading', 'wp-mailinglist'); }
								elseif (!is_uploaded_file(sanitize_text_field(wp_unslash($_FILES['upload']['tmp_name'])))) { $this -> errors['upload'] = __('HTML file could not be uploaded', 'wp-mailinglist'); }
								elseif ($_FILES['upload']['type'] != "text/html") { $this -> errors['upload'] = __('This is not a valid HTML file. Ensure that it has a .html extension', 'wp-mailinglist'); }
									...
									@chmod(sanitize_text_field(wp_unslash($_FILES['upload']['tmp_name'])), 0755);
									
									if ($fh = fopen(sanitize_text_field(wp_unslash($_FILES['upload']['tmp_name'])), "r")) {
										$html = "";

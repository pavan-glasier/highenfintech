...
class wpmlEmail extends wpMailPlugin {
	...
	function validate($data = array()) {
		...
		if (!empty($data)) {
			if (empty($subscriber_id) && empty($user_id)) { $this -> errors['subscriber_id'] = __('No subscriber was specified', 'wp-mailinglist'); }
			//if (empty($mailinglist_id)) { $this -> errors['mailinglist_id'] = __('No mailing list was specified', 'wp-mailinglist'); }
			if (empty($eunique)) { $this -> errors['eunique'] = __('No unique ID was specified', 'wp-mailinglist'); }

...
if (!class_exists('wpmlSubscriber')) {
	class wpmlSubscriber extends wpMailPlugin {
		...
		function email_validate($email = null) {
			...
			if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
				...
				if (!empty($emailvalidationextended)) {
					...
					require($this -> plugin_base() . DS . 'vendors' . DS . 'EmailVerify.class.php');
		...
		function search($data = array()) {
			...
			if (!empty($data)) {
				if (empty($data['searchterm'])) { $this -> errors['searchterm'] = __('Please fill in a searchterm', 'wp-mailinglist'); }
				if (empty($data['searchtype'])) { $this -> errors['searchtype'] = __('Please select a search type', 'wp-mailinglist'); }
		...
		function optin($data = array(), $validate = true, $checkexists = true, $confirm = true, $skipsubscriberupdate = false, $wperror = false) {
			...
			$this -> errors = array();
			$number = (!empty($_REQUEST['uninumber'])) ? (int) $_REQUEST['uninumber'] : false;
			...
			if (!empty($data)) {
				...
				if (empty($data['email'])) { $this -> errors['email'] = esc_html($emailfield -> errormessage); }
				elseif (!$this -> email_validate($data['email'])) { $this -> errors['email'] = esc_html($emailfield -> errormessage); }
					...
					if ($data['id'] = $this -> email_exists($data['email'])) {												
						...
						if (!empty($checkexists) && $checkexists == true && !empty($lists)) {																
							...
							if (!empty($redirecturl)) {
								$this -> render('error', array('errors' => array('email' => esc_html($this -> get_option('subscriberexistsmessage')))), true, 'default');	
		...
		function save($data = array(), $validate = true, $return_query = false, $skipsubscriberupdate = false, $emptyfields = false) {
			...
			if ($validate == true) {			
				...
				if (empty($email)) { $this -> errors['email'] = esc_html($emailfield -> errormessage); }
								...
								if (!empty($data[$field -> slug]) || $data[$field -> slug] == "0" || $field -> type == "file") {																								
									if ($field -> type == "file") {	
										...
										if (!empty($_FILES[$field -> slug]['name'])) {							
											...
											$uploadedfile = map_deep(wp_unslash($_FILES[$field -> slug]), 'sanitize_text_field');
